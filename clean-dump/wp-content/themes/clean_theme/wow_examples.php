...
// Display Custom Post Type with pagination
    <?php query_posts('post_type_name=post&post_status=publish&posts_per_page=2&paged='. get_query_var('paged')); /*<-- Enter name of Custom Post Type and count of posts on page here-->*/ ?>
	<?php if ( have_posts() ) : ?>
...
// How To Display ACF Options
	...
	function my_options_page_settings($options) {
	    $options['title'] = __('My Options', 'foundation');
	    $options['pages'] = array(
            __('Header', 'foundation'),
            __('Sidebar', 'foundation'),
            __('Contacts', 'foundation'),
            __('Footer', 'foundation')
	    );
	    return $options;
	} ?>
